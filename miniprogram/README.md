...
AppID：`wx30d01d25ba6ff5c3`。统一使用本 `miniprogram/` 目录开发、预览和上传。资讯和论坛完整保留，后台控制是否开放。运行 `npm run mini:check` 构建并检查，`npm run mini:preview` 启动微信开发者工具预览。

## 功能与模块开关

- 资讯：图文、搜索分类、分页、详情和来源链接。
- 学习：知识、实践、视频来源与应用场景，收藏与阅读记录。
- 工具：模型/平台查询与详情，复用网站引擎的 Token 费用预算，以及豆包 / DeepSeek「AI眼里的你」画像。
- 我的：微信登录、昵称头像、收藏、社区入口；论坛浏览、发帖、回复和删除自己的内容。

管理后台 → **小程序设置** → 设置资讯、社区、模型库、平台导航、AI 眼里的你五项开关，或勾选「精简模式」，点击保存。默认均开放；精简模式对所有用户关闭这五项，工具区保留 Token 费用计算。开关只影响小程序，网站与原数据不变。配置保存在 SQLite `meta.miniModules`，保存记入 `history` 审计；所有用户得到相同配置，不识别审核者。

- `GET /api/v1/mini/settings` 返回公开模块状态。
- 小程序只使用 `pages/home/index` 承载首页四个内容组件，通过 WXML 自绘底部导航切换，不配置 `app.json.tabBar`，不调用 `wx.switchTab`。资讯关闭时移除资讯按钮，论坛关闭时移除“我的”社区入口。
...
- 模型库与平台导航的列表、详情、收藏入口分别受 models / platforms 控制，使用 `/api/v1/mini/models`、`/api/v1/mini/tools` 及对应详情接口。账号、学习、费用计算仍可使用。网站公开 API 保持原有服务范围；开关不用于保护秘密数据。
...
自动化集成测试使用 `test-results/miniprogram-integration-*/app` 下的临时副本，连接独立的本地测试服务。测试结束会关闭该临时项目并重新打开正式 `miniprogram/` 项目；请勿把临时副本作为日常开发入口。正式项目默认连接 `https://ruming.top`。

头像昵称使用微信原生 `chooseAvatar` 和 `input type="nickname"` 填写能力，由用户选择后保存至本站账号。`wx.login` 仅做身份认证，不会静默同步微信资料；再次登录保留本站已保存的昵称和头像。昵称以表单提交值为准，避免使用输入过程中的未通过微信校验的旧值。真机验证微信昵称推荐、选头像、取消及保存后的重新登录。
