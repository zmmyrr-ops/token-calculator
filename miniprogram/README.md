...
管理后台 → **小程序设置** → 设置资讯、社区、模型库、平台导航四项开关，或勾选「精简模式」，点击保存。默认均开放；精简模式对所有用户关闭这四项，工具区保留 Token 费用计算和 AI 画像。开关只影响小程序，网站与原数据不变。配置保存在 SQLite `meta.miniModules`，保存记入 `history` 审计；所有用户得到相同配置，不识别审核者。
