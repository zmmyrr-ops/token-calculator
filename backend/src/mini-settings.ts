...
export const miniSettingsSchema = z
  .object({ news: z.boolean(), forum: z.boolean(), models: z.boolean().default(true), platforms: z.boolean().default(true), minimalMode: z.boolean().default(false) })
  .strict();
export function miniConfiguration(store: ContentDatabase) {
  return {
    news: true, forum: true, models: true, platforms: true, minimalMode: false,
    ...store.meta<{ news: boolean; forum: boolean; models?: boolean; platforms?: boolean; minimalMode?: boolean; updatedAt?: string }>("miniModules"),
  };
}
export function miniSettings(store: ContentDatabase) {
  const config = miniConfiguration(store);
  return config.minimalMode
    ? { ...config, news: false, forum: false, models: false, platforms: false }
    : config;
}
export function requireMiniModule(
  store: ContentDatabase,
  key: "news" | "forum" | "models" | "platforms",
) {
