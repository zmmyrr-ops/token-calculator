...
登录/注册、昵称与头像、个人资料；论坛支持分类搜索分页、发帖、回复、作者编辑删除；后台“社区管理”可管理内容和用户。初始导入 120 个有标记且不可登录的示例账号、48 篇原创示例话题、96 条关联回复。详见 [用户体系与论坛](docs/13-用户体系与论坛.md)。

- [学习中心、用户管理与视频课程维护](docs/14-学习中心与用户管理.md)

## 微信小程序

`miniprogram/` 是 AI 门道的原生微信小程序（AppID `wx30d01d25ba6ff5c3`），按资讯、学习、工具、我的四个 Tab 组织。复用现有 Node API 和社区账号，无独立模拟内容库。导入、接口、隐私配置及发布说明见 [小程序文档](miniprogram/README.md)。运行 `npm run mini:check` 检查源码包，`npm run mini:test` 在微信开发者工具中进行独立数据库集成测试。
